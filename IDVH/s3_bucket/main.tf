...
locals {
  ...
  lifecycle_rule = var.lifecycle_rule != null ? var.lifecycle_rule : try(local.idvh_config.lifecycle_rule, [])
}
...
module "s3_bucket_raw" {
  ...
  acl           = null
  force_destroy = coalesce(var.force_destroy, try(local.idvh_config.force_destroy, false))
  ...
  server_side_encryption_configuration = {
    rule = {
      apply_server_side_encryption_by_default = {
        sse_algorithm     = var.kms_key_arn != null ? "aws:kms" : try(local.idvh_config.sse_algorithm, "AES256")
