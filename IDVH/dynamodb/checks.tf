locals {
  ...
  replica_kms_missing = [
    ...
    replica.region_name
    if local.replica_kms_required && (replica.kms_key_arn == null || length(trimspace(replica.kms_key_arn)) == 0)
  ]
}
...
check "dynamodb_table_inputs" {
  assert {
    condition = (
      length(trimspace(var.table_config.table_name)) > 0 &&
      length(trimspace(var.table_config.hash_key)) > 0 &&
      length(var.table_config.attributes) > 0 &&
      length(local.attribute_names) == length(distinct(local.attribute_names)) &&
      alltrue([
        for attribute in var.table_config.attributes :
        length(trimspace(attribute.name)) > 0 && contains(local.valid_attribute_types, attribute.type)
      ]) &&
      contains(local.attribute_names, var.table_config.hash_key)
    )
